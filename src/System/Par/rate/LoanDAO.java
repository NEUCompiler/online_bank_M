...
/**
 * A data access object (DAO) providing persistence and search support for Loan
...
public class LoanDAO {
	...
	public void save(Loan transientInstance) {
		log.debug("saving Loan instance");
		try {
	...
	public Loan findById(java.lang.String id) {
		...
		Session session = sessionFactory.openSession();
		try {
			Loan instance = (Loan) session.get(
					"System.Par.rate.Loan", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Loan instance) {
		...
		try {
			List results = getCurrentSession()
					.createCriteria("System.Par.rate.Loan")
	...
	public List findByProperty(String propertyName, Object value) {
		log.debug("finding Loan instance with property: " + propertyName
				+ ", value: " + value);
		try {
			String queryString = "from Loan as model where model."
					+ propertyName + "= ?";
	...
	public List findAll() {
		log.debug("finding all Loan instances");
		ApplicationContext c=new ClassPathXmlApplicationContext("applicationContext.xml");
		sessionFactory = (SessionFactory) c.getBean("sessionFactory");
		Session session = sessionFactory.openSession();
		try {
	...
	public Loan merge(Loan detachedInstance) {
		...
		try {
			Loan result = (Loan) getCurrentSession().merge(detachedInstance);
	...
	public static LoanDAO getFromApplicationContext(ApplicationContext ctx) {
		return (LoanDAO) ctx.getBean("LoanDAO");
	}
